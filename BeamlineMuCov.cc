...
void BeamlineMuCov() {
    ...
    while (true) {
        ...
        if (totalTaggedProtons > 0) {
            ...
            PionAbsKEUnivs[0]->Fill(energyAtVertex, ev.weight);
            PionAbsKEUnivs[1]->Fill(energyAtVertex, ev.weight);
            if (ev.truthPrimaryPDG == 13) {
                PionAbsKEUnivs[0]->Fill(energyAtVertex, -MUON_COMP_UNC * ev.weight); 
                PionAbsKEUnivs[1]->Fill(energyAtVertex, +MUON_COMP_UNC * ev.weight);
            }
            ...
            if (ev.backgroundType == 0 || ev.backgroundType == 1) {
                ...
                PionAbsKEBkgUnivs[0]->Fill(energyAtVertex, ev.weight);
                PionAbsKEBkgUnivs[1]->Fill(energyAtVertex, ev.weight);
                if (ev.truthPrimaryPDG == 13) {
                    PionAbsKEBkgUnivs[0]->Fill(energyAtVertex, -MUON_COMP_UNC * ev.weight);
                    PionAbsKEBkgUnivs[1]->Fill(energyAtVertex, +MUON_COMP_UNC * ev.weight);
                }
            }
    ...
    for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
        ...
        std::vector<std::vector<std::vector<TH1D*>>> TrueRecoAsByBinUniv = {TrueAbsAsByBinNom, TrueScatterAsByBinNom};
    ...
    std::vector<TString> TwoDTitles = {
        "BeamlineMu/MeasureCovariance",
        "BeamlineMu/MeasureFracCovariance",
        "BeamlineMu/MeasureCorrelation",
        "BeamlineMu/AbsCovariance",
        "BeamlineMu/AbsFracCovariance",
        "BeamlineMu/AbsCorrelation",
    ...
    TFile* saveFile = new TFile("/exp/lariat/app/users/epelaez/analysis_abs_scatt/histos/beamlinemu/Measure.root", "RECREATE");
    saveFile->cd();
