...
void BeamlineMuCov() {
    ...
    for (Int_t i = 0; i < NumEntries; ++i) {
        ...
        if (totalTaggedProtons > 0) {
            ...
            PionAbsKEUnivs[0]->Fill(energyAtVertex);
            PionAbsKEUnivs[1]->Fill(energyAtVertex);
            if (truthPrimaryPDG == 13) {
                PionAbsKEUnivs[0]->Fill(energyAtVertex, -MUON_COMP_UNC); 
                PionAbsKEUnivs[1]->Fill(energyAtVertex, +MUON_COMP_UNC);
            }
            ...
            if (backgroundType == 0 || backgroundType == 1) {
                ...
                PionAbsKEBkgUnivs[0]->Fill(energyAtVertex);
                PionAbsKEBkgUnivs[1]->Fill(energyAtVertex);
                if (truthPrimaryPDG == 13) {
                    PionAbsKEBkgUnivs[0]->Fill(energyAtVertex, -MUON_COMP_UNC);
                    PionAbsKEBkgUnivs[1]->Fill(energyAtVertex, +MUON_COMP_UNC);
                }
            }
