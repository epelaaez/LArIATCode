...
void EnergyRecoCov() {
    ...
    while (true) {
        ...
        for (size_t iHit = 0; iHit < std::min(nhits, kMaxHits); ++iHit) {
            // if (ev.fHitPlane.at(iHit) == 1) continue;
        ...
        if (ev.backgroundType == 0 || ev.backgroundType == 1) {
            hTruePionAbsKENom->Fill(ev.truthPrimaryVertexKE * 1000, ev.weight);
            for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) TruePionAbsKEUnivs[iUniv]->Fill(ev.truthPrimaryVertexKE * 1000, ev.weight);
        ...
        if (totalTaggedPions > 0) {
            ...
            if (ev.backgroundType == 0 || ev.backgroundType == 1) {
                if (TrueEnergyBin != -1) {
                    TrueAbsAsByBinNom.at(TrueEnergyBin).at(1)->Fill(energyAtVertex, ev.weight);
                    TrueAbsAsByBinUnivs.at(0).at(TrueEnergyBin).at(1)->Fill(energyAtVertex + energyAtVertex_unc, ev.weight);
                    TrueAbsAsByBinUnivs.at(1).at(TrueEnergyBin).at(1)->Fill(energyAtVertex - energyAtVertex_unc, ev.weight);
                }
            } else if (ev.backgroundType == 6 || ev.backgroundType == 12) {
                if (TrueEnergyBin != -1) {
        ...
        if (totalTaggedProtons > 0) {
            ...
            // Add weights to abs
            hPionAbsKENom->Fill(energyAtVertex, ev.weight);
            PionAbsKEUnivs[0]->Fill(energyAtVertex + energyAtVertex_unc, ev.weight);
            PionAbsKEUnivs[1]->Fill(energyAtVertex - energyAtVertex_unc, ev.weight);
            if (ev.backgroundType == 0 || ev.backgroundType == 1) {
                if (TrueEnergyBin != -1) {
                    TrueAbsAsByBinNom.at(TrueEnergyBin).at(0)->Fill(energyAtVertex, ev.weight);
                    TrueAbsAsByBinUnivs.at(0).at(TrueEnergyBin).at(0)->Fill(energyAtVertex + energyAtVertex_unc, ev.weight);
                    TrueAbsAsByBinUnivs.at(1).at(TrueEnergyBin).at(0)->Fill(energyAtVertex - energyAtVertex_unc, ev.weight);
                }
            } else if (ev.backgroundType == 6 || ev.backgroundType == 12) {
                if (TrueEnergyBin != -1) {
                ...
                hPionAbsKEBkgNom->Fill(energyAtVertex, ev.weight);
                PionAbsKEBkgUnivs[0]->Fill(energyAtVertex + energyAtVertex_unc, ev.weight);
                PionAbsKEBkgUnivs[1]->Fill(energyAtVertex - energyAtVertex_unc, ev.weight);
            }
    ...
    std::vector<TString> TwoDTitles = {
        ...
        "EnergyReco/AbsCovariance",
        "EnergyReco/AbsFracCovariance",
        "EnergyReco/AbsCorrelation",
    ...
    TFile* saveFile = new TFile("/exp/lariat/app/users/epelaez/analysis_abs_scatt/histos/energyreco/Measure.root", "RECREATE");
    saveFile->cd();
