...
void FakeData1() {
    ...
    for (int iSignal = 0; iSignal < NUM_SIGNAL_TYPES; ++iSignal) {
        for (int iBin = 0; iBin < NUM_BINS_KE; ++iBin) {
            ...
            hSignal->SetBinContent(index + 1,  xsec); TotalEventsHistos[iSignal]->SetBinContent(iBin + 1,  xsec);
        }
    }
    ...
    for (int iSignal = 0; iSignal < NUM_SIGNAL_TYPES; ++iSignal) {
        for (int iBin = 0; iBin < NUM_BINS_KE; ++iBin) {
            ...
            hMeasure->SetBinContent(index + 1, xsec); RecoSignals[iSignal]->SetBinContent(iBin + 1, xsec);
        }
    }
