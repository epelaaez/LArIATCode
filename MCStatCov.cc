    ...
    for (Int_t i = 0; i < NumEntries; ++i) {
        ...
        if (totalTaggedPions > 0) {
            ...
            for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
                ...
                if (backgroundType == 0 || backgroundType == 1) {
                    if (TrueEnergyBin != -1) TrueAbsAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(1)->Fill(energyAtVertex, weights[iUniv]);
                } else if (backgroundType == 6 || backgroundType == 12) {
        ...
        if (totalTaggedProtons > 0) {
            ...
            for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
                PionAbsKEUnivs[iUniv]->Fill(energyAtVertex, weights[iUniv]);
                if (backgroundType == 0 || backgroundType == 1) {
                    if (TrueEnergyBin != -1) TrueAbsAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(0)->Fill(energyAtVertex, weights[iUniv]);
                } else if (backgroundType == 6 || backgroundType == 12) {
                    if (TrueEnergyBin != -1) TrueScatterAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(0)->Fill(energyAtVertex, weights[iUniv]);
                } else {
                    PionAbsKEBkgUnivs[iUniv]->Fill(energyAtVertex, weights[iUniv]);
                }
            }
