...
void MCStatCov() {
    // This module generates all relevant covariance matrices coming from MC stat systematics
    ...
    while (true) {
        ...
        // Get weights 
        std::vector<int> weights(NUM_UNIVERSES);
        double avg = 0;
        for (int i = 0; i < NUM_UNIVERSES; ++i) {
            rand->SetSeed(event * 1000 + i);
            weights[i] = rand->Poisson(1);
        }
        ...
        if (totalTaggedPions > 0) {
            ...
            for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
                ...
                if (ev.backgroundType == 0 || ev.backgroundType == 1) {
                    if (TrueEnergyBin != -1) TrueAbsAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(1)->Fill(energyAtVertex, weights[iUniv] * ev.weight);
                } else if (ev.backgroundType == 6 || ev.backgroundType == 12) {
        ...
        if (totalTaggedProtons > 0) {
            ...
            for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
                PionAbsKEUnivs[iUniv]->Fill(energyAtVertex, weights[iUniv] * ev.weight);
                if (ev.backgroundType == 0 || ev.backgroundType == 1) {
                    if (TrueEnergyBin != -1) TrueAbsAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(0)->Fill(energyAtVertex, weights[iUniv] * ev.weight);
                } else if (ev.backgroundType == 6 || ev.backgroundType == 12) {
                    if (TrueEnergyBin != -1) TrueScatterAsByBinUnivs.at(iUniv).at(TrueEnergyBin).at(0)->Fill(energyAtVertex, weights[iUniv] * ev.weight);
                } else {
                    PionAbsKEBkgUnivs[iUniv]->Fill(energyAtVertex, weights[iUniv] * ev.weight);
                }
            }
    ...
    for (int iUniv = 0; iUniv < NUM_UNIVERSES; ++iUniv) {
        ...
        std::vector<TH1D*> TotalEventsUniv = {hTruePionAbsKENom, hTruePionScatterKENom};
    ...
    std::vector<TString> TwoDTitles = {
        "MCStat/MeasureCovariance",
        "MCStat/MeasureFracCovariance",
        "MCStat/MeasureCorrelation",
        "MCStat/AbsCovariance",
        "MCStat/AbsFracCovariance",
        "MCStat/AbsCorrelation",
        "MCStat/ScatterCovariance",
        "MCStat/ScatterFracCovariance",
        "MCStat/ScatterCorrelation"
    };
    ...
    TFile* saveFile = new TFile("/exp/lariat/app/users/epelaez/analysis_abs_scatt/histos/mcstat/Measure.root", "RECREATE");
    saveFile->cd();
