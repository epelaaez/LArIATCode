...
void Unfold() {
    ...
    TString SaveDir = "/exp/lariat/app/users/epelaez/comps/analysis1/figs/Unfold/";

    int N = NUM_SIGNAL_TYPES * NUM_BINS_KE;

    // Load nominal histograms
    std::unique_ptr<TFile> NominalFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/nominal/Reco.root"));
    ...
    std::unique_ptr<TFile> GeneratorFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/generator/Measure.root"));
    TH2D* GeneratorCovariance = (TH2D*) GeneratorFile->Get("hMeasureCovMatrix");

    // Energy reconstruction covariance
    std::unique_ptr<TFile> EnergyRecoFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/energyreco/Measure.root"));
    TH2D* EnergyRecoCovariance = (TH2D*) EnergyRecoFile->Get("hMeasureCovMatrix");

    // Beam line muon covariance
    std::unique_ptr<TFile> BeamlineMuFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/beamlinemu/Measure.root"));
    TH2D* BeamlineMuCovariance = (TH2D*) BeamlineMuFile->Get("hMeasureCovMatrix");

    // Beam line electron covariance
    std::unique_ptr<TFile> BeamlineElFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/beamlineel/Measure.root"));
    TH2D* BeamlineElCovariance = (TH2D*) BeamlineElFile->Get("hMeasureCovMatrix");

    // MC stat covariance
    std::unique_ptr<TFile> MCStatFile(TFile::Open("/exp/lariat/app/users/epelaez/comps/analysis1/histos/mcstat/Measure.root"));
