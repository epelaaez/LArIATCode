...
def plot_classification_error(history, metric):
    ...
    plt.savefig("figs/abs/classification_error.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_confusion_matrix(model, X_test, y_test, labels):
    ...
    plt.savefig("figs/abs/confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()

def plot_feature_importance(model):
    xgb.plot_importance(model, importance_type="gain")
    plt.title("Feature Importance by Gain")
    plt.savefig("figs/abs/feature_importance.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_roc_curves(model, X_test, y_test, classes):
    ...
    if len(classes) == 2:
        ...
        plt.legend()
        plt.savefig("figs/abs/roc_curves.png", dpi=300, bbox_inches="tight")
        plt.close()
...
if (__name__ == "__main__"):
    ...
    for ci, trees in enumerate(per_class_trees):
        convert.convert_model(
            ...
            output_xml=f"model/abs_model_class_{ci}.xml"
        )
    ...
    plot_confusion_matrix(model, X_test, y_test, ["abs 0p", "scatter", "other"])
